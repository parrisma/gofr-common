...
if ! docker image inspect gofr-openwebui:prod >/dev/null 2>&1; then
    ...
    bash "${COMMON_ROOT}/docker/openwebui/build.sh"
...
if ! docker image inspect gofr-n8n:prod >/dev/null 2>&1; then
    ...
    bash "${COMMON_ROOT}/docker/n8n/build.sh"
...
cd "$SCRIPT_DIR"
docker compose -f openwebui/tools-compose.prod.yml down 2>/dev/null || true
docker compose -f openwebui/tools-compose.prod.yml up -d
