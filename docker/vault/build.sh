...
IMAGE_NAME="gofr-vault"
DOCKERFILE="docker/vault/Dockerfile.dev"
TITLE="Vault Dev"
...
echo "Dockerfile: ${COMMON_ROOT}/${DOCKERFILE}"
echo ""

cd "$COMMON_ROOT"
ensure_base_image_if_needed "$DOCKERFILE"
docker build \
    ${NO_CACHE} \
    -f "$DOCKERFILE" \
    -t "${IMAGE_NAME}:dev" \
